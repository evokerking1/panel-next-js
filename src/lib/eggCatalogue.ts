...
async function cloneOrPullRepo(repoUrl: string, targetDir: string): Promise<void> {
  ...
  if (!fs.existsSync(targetDir) || !isGitRepo(targetDir)) {
    if (fs.existsSync(targetDir)) {
      console.info(`Store: removing broken directory at ${targetDir} before re-cloning`);
      fs.rmSync(targetDir, { recursive: true, force: true });
    }
    console.info(`Store: cloning ${repoUrl}`);
    await execAsync(`git clone "${repoUrl}" "${targetDir}"`, { env });
    return;
  }

  console.info(`Store: pulling latest for ${path.basename(targetDir)}`);
...
async function updateRepos(): Promise<void> {
  if (!isGitAvailable()) {
    console.warn('Store: git not found -- cannot clone egg repos');
    return;
  }
  ...
  results.forEach((r, i) => {
    if (r.status === 'rejected') {
      console.warn(`Store: ${REPOS[i].dir} failed: ${r.reason?.message || r.reason}`);
    }
  });
}
...
function rebuildCatalogue(): void {
  ...
  lastBuilt = Date.now();
  console.info(`Store: catalogue built -- ${catalogue.length} images`);
}

function scheduleAutoUpdate(): void {
  if (updateTimer) clearInterval(updateTimer);
  updateTimer = setInterval(async () => {
    console.info('Store: auto-updating egg repos');
    await updateRepos();
    rebuildCatalogue();
  }, TWO_DAYS_MS);
