...
import InstallBanner from '@/components/server/install-banner'
...
function FileActionMenu({
  ...
  openImagePreview: (name: string) => Promise<void>
  openTextEditor: (name: string) => void
  setDeleteTarget: (file: FileEntry | null) => void
  ...
  return (
    <>
      {file.type === 'file' && !isImageFile(file.name) && (
        <button onClick={() => { openTextEditor(file.name); closeMenu() }} className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-xs font-medium text-neutral-700 transition hover:bg-neutral-50 dark:text-neutral-300 dark:hover:bg-white/5">
          <Pencil className="w-3.5 h-3.5" />
          Edit
        </button>
      )}
      {file.type === 'file' && isImageFile(file.name) && (
        <button onClick={() => { void openImagePreview(file.name); closeMenu() }} className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-xs font-medium text-neutral-700 transition hover:bg-neutral-50 dark:text-neutral-300 dark:hover:bg-white/5">
          <Eye className="w-3.5 h-3.5" />
          Preview
        </button>
      )}
      <button onClick={() => { void copyPath(fullPath(file.name)); closeMenu() }} className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-xs font-medium text-neutral-700 transition hover:bg-neutral-50 dark:text-neutral-300 dark:hover:bg-white/5">
        <Copy className="w-3.5 h-3.5" />
        Copy path
      </button>
      <button onClick={() => { setRenameTarget(file); setRenameName(file.name); closeMenu() }} className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-xs font-medium text-neutral-700 transition hover:bg-neutral-50 dark:text-neutral-300 dark:hover:bg-white/5">
        <Pencil className="w-3.5 h-3.5" />
        Rename
      </button>
      {file.type === 'file' && (
        <button onClick={() => { downloadFile(file.name); closeMenu() }} className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-xs font-medium text-neutral-700 transition hover:bg-neutral-50 dark:text-neutral-300 dark:hover:bg-white/5">
          <Download className="w-3.5 h-3.5" />
          Download
        </button>
      )}
      <button onClick={() => { setDeleteTarget(file); closeMenu() }} className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-xs font-medium text-red-600 transition hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-500/10">
