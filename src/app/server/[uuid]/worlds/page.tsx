...
import ServerLoadingPanel from '@/components/server/server-loading-panel'
import { useServerSummary } from '@/components/server/use-server-summary'
...
export default function WorldsPage({ params }: { params: Promise<{ uuid: string }> }) {
  ...
  const { server, features, installing, status, loading } = useServerSummary<ServerInfo>(uuid)
  ...
  if (loading || worldsLoading || !server) {
    return <ServerLoadingPanel actionSkeletons={2} />
  }
    ...
    <PanelLayout>
      ...
      <div className="panel-page panel-stack">
        ...
        {worlds.length === 0 ? (
          ...
          <>
            <div className="panel-card hidden lg:block overflow-hidden shadow">
